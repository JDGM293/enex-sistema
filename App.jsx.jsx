...
import { dbGetClientes, dbUpsertCliente, dbDeleteCliente, dbGetWR, dbUpsertWR, dbGetAgentes, dbUpsertAgente, dbDeleteAgente, dbGetOficinas, dbUpsertOficina, dbDeleteOficina, dbGetTarifas, dbUpsertTarifa, dbDeleteTarifa, dbGetConsolidaciones, dbUpsertConsolidacion, dbLogActividad, dbGetActividad } from "./supabase";
...
@media print{
  .sb,.topbar,.wr-toolbar,.pag,.rp,.mft{display:none!important}
  .modal{box-shadow:none;border:none}
  .ov{position:static;background:none;backdrop-filter:none;padding:0}
  body{background:#fff}
}
...
// ─── NUMBERING BUILDERS ───────────────────────────────────────────────────────
const buildWRNum=(od,oc,dd,dc,seq,tipo=1,secInicio=1,empresa="1N")=>{
  const s=String(seq+(secInicio-1)).padStart(7,"0");
  switch(tipo){
    case 1: return s; // 7 dígitos puros
    case 2: return `${empresa}${String(od).padStart(2,"0")}${oc.slice(0,2).toUpperCase()}${String(dd).padStart(2,"0")}${dc.slice(0,2).toUpperCase()}${s}`;
    case 3: default: return `${String(od).padStart(2,"0")}${oc.slice(0,2).toUpperCase()}${String(dd).padStart(2,"0")}${dc.slice(0,2).toUpperCase()}${s}`;
  }
};
const buildConsolNum=(dest,seq,tipo=1,secInicio=1,origDial="01",destDial="58",tipoEnvio="AE")=>{
  const s=String(seq+(secInicio-1)).padStart(7,"0");
  const te=tipoEnvio.replace(/[^A-Z]/gi,"").slice(0,2).toUpperCase()||"AE";
  const y=new Date().getFullYear().toString().slice(2);
  const m=String(new Date().getMonth()+1).padStart(2,"0");
  const d=String(new Date().getDate()).padStart(2,"0");
  switch(tipo){
    case 1: return s;
    case 2: return `${String(origDial).padStart(2,"0")}${String(destDial).padStart(2,"0")}${s}${te}`;
    case 3: default: return `${y}${m}${d}${dest.slice(0,3).toUpperCase()}${s}`;
  }
};

// ─── STATIC DATA ──────────────────────────────────────────────────────────────
const WR_STATUSES=[
  {code:"1",label:"Recibido en Puerta",cls:"s1"},{code:"2",label:"Registrado",cls:"s1"},
  {code:"2.1",label:"Origen",cls:"s2"},{code:"2.2",label:"Espera Confirmación",cls:"s5"},
  {code:"2.3",label:"Reempacado",cls:"s6"},
  {code:"3",label:"Confirmado",cls:"s3"},{code:"4",label:"Consolidado",cls:"s3"},
  {code:"5",label:"Entregado a Línea",cls:"s2"},{code:"6",label:"Aduana Salida",cls:"s5"},
  {code:"6.1",label:"Auditoría Aduana",cls:"s4"},{code:"6.2",label:"Liberado Salida",cls:"s3"},
  {code:"7",label:"En Tránsito",cls:"s2"},{code:"7.1",label:"Aduana Tránsito",cls:"s5"},
  {code:"8",label:"Tránsito a Destino",cls:"s2"},{code:"9",label:"Ciudad Destino",cls:"s6"},
  {code:"9.1",label:"Aduana Destino",cls:"s5"},{code:"10",label:"Almacén Destino",cls:"s6"},
  {code:"10.1",label:"Faltante Descarga",cls:"s4"},{code:"10.2",label:"Sobrante Descarga",cls:"s5"},
  {code:"11",label:"Listo para Entrega",cls:"s3"},{code:"12",label:"Entregado",cls:"s3"},
  {code:"12C",label:"Entregado/Cobrado",cls:"s3"},{code:"12P",label:"Entregado/Por Cobrar",cls:"s5"},
];
...
// ─── WR ROW ────────────────────────────────────────────────────────────────────
const WRRow=({w,sel,onClick,unitL,unitW,dimOpen,onDimToggle})=>{
  const isIn=unitL==="in";
  const isLb=unitW==="lb";
  ...
  return (
    <tr className={sel?"sel":""} onClick={onClick}>
      ...
      <td><span className="br-b">{w.branch}</span></td>
      ...
      <td style={{maxWidth:110,overflow:"hidden",textOverflow:"ellipsis"}}>{w.descripcion||"—"}</td>
      ...
      <td><span className="c-trk" style={{fontSize:10}}>{w.tracking||"—"}</span></td>
...
// ─── WR TABLE COMPONENT ────────────────────────────────────────────────────────
const PAGE_SIZE=50;
const WRTable=({rows,selId,onSelect,unitL,unitW,onSort,sortCol,sortDir,dimOpen,onDimToggle,page,onPage})=>{
        ...
        <table className="wt">
          <thead>
            <tr>
              ...
              <th style={{minWidth:130}}>Descripción</th>
              ...
              <th style={{minWidth:160}}>Tracking</th>
              <th style={{width:120,maxWidth:120}}>Estado</th>
...
const emptyCaja=()=>({
  ...
  descripcion:"", tipoEmbalaje:"Caja Cartón",
});
const emptyWRF=()=>({
  ...
  tipoPago:"Prepago",tipoEnvio:"Aéreo Express",
  notas:"",cargos:[],
...
const ClientModal=({initial,onClose,onSave,title,agentes=[],oficinas=[]})=>{
  const [f,setF]=useState({password:"",...initial});
  const ff=(k,v)=>setF(p=>({...p,[k]:v}));
  const rolReqAgente=["E","E1"].includes(f.rol);       // Agente / Vendedor Agente → necesita agente padre
  const rolReqOficina=["G","G1"].includes(f.rol);  // Oficina / Vendedor Oficina → necesita oficina padre
  return (
    <div className="ov" onClick={onClose}>
      <div className="modal mmd" onClick={e=>e.stopPropagation()}>
        ...
        {/* Pertenece a — solo para clientes */}
        {f.tipo==="cliente"&&(
        ...
        {/* Selector de Agente padre para E (Agente) y E1 (Vendedor/Agente) */}
        {f.tipo==="usuario"&&rolReqAgente&&(
          <div className="fgrid g2" style={{marginBottom:10}}>
            <div className="fg"><div className="fl">{f.rol==="E"?"Agente Matriz (si aplica)":"Agente al que pertenece *"}</div>
              <select className="fs" value={f.agenteId||""} onChange={e=>ff("agenteId",e.target.value)}>
                <option value="">— Selecciona un Agente —</option>
                {agentes.map(a=><option key={a.id} value={a.id}>{a.codigo} — {a.nombre}</option>)}
              </select>
        ...
        {/* Selector de Oficina padre para G (Oficina) y G1 (Vendedor Oficina) */}
        {f.tipo==="usuario"&&rolReqOficina&&(
          <div className="fgrid g2" style={{marginBottom:10}}>
            <div className="fg"><div className="fl">{f.rol==="G"?"Oficina Matriz (si aplica)":"Oficina a la que pertenece *"}</div>
              <select className="fs" value={f.oficinaId||""} onChange={e=>ff("oficinaId",e.target.value)}>
                <option value="">— Selecciona una Oficina —</option>
                {oficinas.map(o=><option key={o.id} value={o.id}>{o.codigo} — {o.nombre}</option>)}
              </select>
        ...
        {/* Password solo para usuarios */}
        {f.tipo==="usuario"&&(
          <div className="fgrid g2" style={{marginBottom:10}}>
            <div className="fg"><div className="fl">Contraseña inicial *</div>
              <input className="fi" type="password" value={f.password||""} onChange={e=>ff("password",e.target.value)} placeholder="Mínimo 6 caracteres"/>
            </div>
            <div className="fg"><div className="fl" style={{fontSize:10,color:"var(--t3)"}}>El usuario podrá cambiarla al ingresar.</div></div>
          </div>
        )}
        ...
        <div className="sdiv">ACCESO AL SISTEMA</div>
        <div className="fgrid g2" style={{marginBottom:10}}>
          <div className="fg"><div className="fl">Contraseña inicial *</div>
            <input className="fi" type="password" value={f.password||""} onChange={e=>ff("password",e.target.value)} placeholder="Mínimo 6 caracteres"/>
          </div>
          <div className="fg" style={{display:"flex",alignItems:"center"}}><div className="fl" style={{fontSize:10,color:"var(--t3)"}}>El {f.tipo==="usuario"?"usuario":"cliente"} podrá cambiarla al ingresar por primera vez.</div></div>
        </div>
        <div className="mft"><button className="btn-s" onClick={onClose}>Cancelar</button><button className="btn-p" onClick={()=>onSave(f)}>Guardar ✅</button></div>
      </div>
    </div>
  );
};
  ...
  return (
    <div style={{background:"var(--bg2)",border:"1px solid var(--b1)",borderRadius:10,padding:"14px 16px",marginBottom:14}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:10}}>
        <input className="fi" style={{flex:1,fontFamily:mono?"'DM Mono',monospace":"inherit"}}
          value={v} onChange={e=>setV(e.target.value)}
          onKeyDown={e=>{if(e.key==="Enter"&&v.trim()){onAdd(v.trim());setV("");}}}
          placeholder={placeholder}/>
        <button className="btn-p" style={{padding:"6px 14px"}} onClick={()=>{if(v.trim()){onAdd(v.trim());setV("");}}} >+ Agregar</button>
      </div>
...
const emptyConsol=()=>({
  destino:"VL", tipoEnvio:"Aéreo Express",
...
export default function ENEXSystem(){
  ...
  const [CHARGES_OPT,setChargesOpt]=useState(CHARGES_INIT);
  const [CONTAINER_TYPES,setContainerTypes]=useState(CONTAINER_TYPES_INIT);
  const [COUNTRIES,setCountries]=useState(COUNTRIES_INIT);
  ...
  const [actFilter,setActFilter]=useState("todos");
  ...
  const [cfgNewCont,setCfgNewCont]=useState("");
  const [agentes,setAgentes]=useState([
    {id:"AG-01",nombre:"ENEX Matriz",codigo:"01",ciudad:"",pais:"Venezuela",tel:"",email:"",contacto:"",notas:"",activo:true},
  ]);
  ...
  const [tarifas,setTarifas]=useState([
    {id:"T-001",paisOrig:"01",ciudadOrig:"MI",paisDest:"58",ciudadDest:"VL",tipoEnvio:"Aéreo Express",porLb:1.8,porFt3:28,min:25,moneda:"USD",activo:true},
    {id:"T-002",paisOrig:"01",ciudadOrig:"MI",paisDest:"58",ciudadDest:"VL",tipoEnvio:"Aéreo Económico",porLb:1.4,porFt3:22,min:20,moneda:"USD",activo:true},
    {id:"T-003",paisOrig:"01",ciudadOrig:"MI",paisDest:"58",ciudadDest:"VL",tipoEnvio:"Marítimo FCL",porLb:0.6,porFt3:10,min:50,moneda:"USD",activo:true},
    {id:"T-004",paisOrig:"01",ciudadOrig:"MI",paisDest:"58",ciudadDest:"VL",tipoEnvio:"Marítimo LCL",porLb:0.8,porFt3:14,min:40,moneda:"USD",activo:true},
    {id:"T-005",paisOrig:"01",ciudadOrig:"MI",paisDest:"58",ciudadDest:"VL",tipoEnvio:"Terrestre",porLb:0.5,porFt3:8,min:30,moneda:"USD",activo:true},
  ]);
  const [showNewTarifa,setShowNewTarifa]=useState(false);
  const [editTarifa,setEditTarifa]=useState(null);
  const [tafForm,setTafForm]=useState({paisOrig:"01",ciudadOrig:"MI",paisDest:"58",ciudadDest:"VL",tipoEnvio:"Aéreo Express",porLb:"",porFt3:"",min:"",moneda:"USD"});
  ...
  const [calcForm,setCalcForm]=useState({largo:"",ancho:"",alto:"",pesoLb:"",unitDim:"in",origPais:"01",origCiudad:"MI",destPais:"58",destCiudad:"VL",tipoEnvio:"Aéreo Express"});
  ...
  useEffect(()=>{
    const load=async()=>{
      const [cls,wrs,ags,ofs,tfs,cons,acts]=await Promise.all([
        dbGetClientes(),dbGetWR(),dbGetAgentes(),dbGetOficinas(),
        dbGetTarifas(),dbGetConsolidaciones(),dbGetActividad(),
      ]);
      if(cls.length>0)setClients(cls);
      if(wrs.length>0)setWrList(wrs);
  ...
  const logAction=(action,detail="")=>{
    setActLog(p=>[{ts:new Date(),user:CU.id||"admin",role:CU.rol,action,detail},...p.slice(0,199)]);
    dbLogActividad(CU.id||"U-001",CU.rol,action,detail);
  };

  const hasPerm=p=>ROLE_DEFS[CU.rol]?.perms.includes(p);
  const canEdit=hasPerm("crear_wr")||hasPerm("editar_wr");
  const canAdmin=CU.rol==="A";
  const canStatus=hasPerm("status_origen")||hasPerm("status_destino")||["A","B","C"].includes(CU.rol);
  ...
  const doScan=()=>{
    ...
    if(!already){
      setScanLog(p=>[{tracking:scanV.trim(),carrier:scanCarrier||"—",ts:new Date(),registered:false},...p]);
    }
    setScanV("");
  };
  ...
  const wrNumPrev=buildWRNum(OFFICE_CONFIG.origCountry,OFFICE_CONFIG.origCity,OFFICE_CONFIG.destCountry,OFFICE_CONFIG.destCity,wrList.length+1,wrNumTipo,wrSecInicio,empresaSlug);
  const seqPrev=String(wrList.length+1).padStart(8,"0");
  ...
  const submitWR=()=>{
    ...
    const n={
      ...
      fecha:now,
      consignee:wrf.consignee,casillero:wrf.casillero,
      ...
      shipper:wrf.remitente,usuario:CU.id,foto:false,prealerta:false,
      cargos:wrf.cargos,
    };
  ...
  const renderWRToolbar=()=>(
    <div className="wr-toolbar">
      {/* Nuevo WR — IZQUIERDA */}
      {hasPerm("crear_wr")&&<button className="btn-p" onClick={()=>setShowNewWR(true)}>+ Nuevo WR</button>}
      {/* Búsqueda avanzada */}
  ...
  const renderDash=()=>(
    <>
      ...
      <div className="dash-grid" style={{flex:1,minHeight:0,overflow:"hidden",padding:"0 16px 14px"}}>
        ...
        <div className="rp">
          <div className="card">
            <div className="card-tt">🔔 Alertas <span className="card-sub">3 urgentes</span></div>
            {[{cls:"alt-e",ic:"🛃",t:"Retención Aduanal",b:"WR en aduana — docs requeridos"},
              {cls:"alt-w",ic:"📦",t:"Faltante en Descarga",b:"2 bultos no recibidos"},
              {cls:"alt-i",ic:"✅",t:"Listos para Entrega",b:`${stats.ready} envíos esperan al cliente`},
              {cls:"alt-ok",ic:"💵",t:"Por Cobrar",b:`${stats.entregadoPorCobrar} envíos pendientes de cobro`},
            ].map((a,i)=>(
        ...
        <div className="card">
          ...
          {Object.keys(histByDay).length===0
            ...
            :Object.entries(histByDay).map(([day,items])=>{
              const open=openDays[day]!==false; // default open
              return (
                <div key={day} style={{marginBottom:8,border:"1px solid var(--b1)",borderRadius:8,overflow:"hidden"}}>
                  ...
                  <div onClick={()=>toggleDay(day)} style={{background:"var(--bg4)",padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",borderBottom:open?"1px solid var(--b1)":"none"}}>
                    ...
                    {canAdmin&&<button className="btn-d" style={{fontSize:9,padding:"2px 8px"}} onClick={e=>{e.stopPropagation();setScanLog(p=>p.filter(s=>fmtDate(s.ts)!==day||!s.registered));}}>🗑 Borrar día</button>}
                  </div>
                  {open&&(
  ...
  const renderNewWRModal=()=>(
    <div className="ov" onClick={()=>setShowNewWR(false)}>
      <div className="modal mxl" onClick={e=>e.stopPropagation()}>
        <div className="mhd">
          <div className="mt">📦 Nuevo Warehouse Receipt — {wrf.cajas.length} {wrf.cajas.length===1?"caja":"cajas"}</div>
          <button className="mcl" onClick={()=>setShowNewWR(false)}>✕</button>
        </div>

        {/* N° WR AUTOMÁTICO */}
        <div className="wr-builder">
          <div style={{fontSize:10,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:"var(--navy)",marginBottom:8}}>NÚMERO DE WR — GENERADO AUTOMÁTICAMENTE</div>
          ...
          <div className="seg-leg"><span>🔵 País Origen</span><span>🟢 Ciudad Origen</span><span>🔵 País Destino</span><span>🟢 Ciudad Destino</span><span>🟡 Secuencia</span></div>
          <div style={{display:"flex",gap:16,justifyContent:"center",fontSize:11,color:"var(--t2)"}}>
            <span>📤 <strong>{OFFICE_CONFIG.origCity}</strong></span><span>→</span>
            <span>📥 <strong>{OFFICE_CONFIG.destCity}</strong></span><span>·</span>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
          <div>
            ...
            <div className="fgrid" style={{gap:8}}>
              <div className="fg"><div className="fl">Nombre / Empresa</div><input className="fi" value={wrf.remitente} onChange={e=>sw("remitente",e.target.value)} placeholder="Amazon.com / Empresa remitente"/></div>
        ...
        <div className="fgrid g3" style={{marginBottom:14}}>
          ...
          <div className="fg"><div className="fl">Tipo de Envío</div><select className="fs" value={wrf.tipoEnvio} onChange={e=>sw("tipoEnvio",e.target.value)}>{SEND_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
          <div className="fg"><div className="fl">Tipo Cliente (Auto)</div><div className="fi ro">{ROLE_DEFS[CU.rol]?.name}</div></div>
        </div>
        ...
        <div style={{display:"flex",gap:10,marginBottom:14,padding:"8px 14px",background:"var(--bg4)",borderRadius:8,border:"1px solid var(--b1)",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:4}}>
            {["pulg.","cm"].map(u=><button key={u} className={`btn-${(u==="pulg."?wrf.unitDim==="in":wrf.unitDim==="cm")?"p":"s"}`} style={{padding:"4px 10px",fontSize:11}} onClick={()=>sw("unitDim",u==="pulg."?"in":"cm")}>{u}</button>)}
          </div>
          <span style={{fontSize:11,fontWeight:600,color:"var(--navy)",marginLeft:12}}>Unidades de peso:</span>
          <div style={{display:"flex",gap:4}}>
            {["lb","kg"].map(u=><button key={u} className={`btn-${wrf.unitPeso===u?"p":"s"}`} style={{padding:"4px 10px",fontSize:11}} onClick={()=>sw("unitPeso",u)}>{u}</button>)}
          </div>
        </div>
              ...
              <div style={{padding:"12px 14px"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"60px 120px 1fr 1fr 1fr 1fr 1fr 1fr 1fr",gap:8,marginBottom:10}}>
                  ...
                  <div className="fg">
                    ...
                    <select className="fs" value={caja.tipoEmbalaje} onChange={e=>swCaja(idx,"tipoEmbalaje",e.target.value)} style={{fontSize:11}}>
                ...
                <div className="fg">
                  ...
                  <input className="fi" value={caja.descripcion} onChange={e=>swCaja(idx,"descripcion",e.target.value)} placeholder="Ej: Electrónica, ropa, repuestos…"/>
                </div>
              </div>
            </div>
          );
        ...
        <div className="mft">
          <button className="btn-s" onClick={()=>setShowNewWR(false)}>Cancelar</button>
          <button className="btn-c">👁 Vista Previa</button>
          <button className="btn-p" onClick={submitWR}>Registrar WR ✅</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="modal mlg" onClick={e=>e.stopPropagation()}>
        <div className="mhd">
          ...
          <div style={{display:"flex",gap:6}}>
            {canEdit&&<button className="btn-s" style={{fontSize:10,padding:"4px 10px"}}>✏️ Editar</button>}
        ...
        <div className="wr-doc">
          <div className="wr-doc-hd">
            <div>
              <div className="wr-co">ENEX</div>
              <div className="wr-co-info">International Courier · {selWR.branch}<br/>Av. Principal, Torre Empresarial, Caracas, Venezuela<br/>+58 212 800-0000 · ops@enex.com</div>
            </div>
            <div>
              <div className="wr-num-d">{selWR.id}</div>
              <div className="wr-num-meta">
                {selWR.origCity} → {selWR.destCity}<br/>
                Fecha: {fmtDate(selWR.fecha)} {fmtTime(selWR.fecha)}<br/>
                Usuario: {selWR.usuario}
              </div>
              <div style={{marginTop:6}}><StBadge st={selWR.status}/></div>
            </div>
          </div>
          <div className="w2c">
            <div className="wb"><div className="wb-t">📤 Información del Remitente</div>
              {[["Nombre/Empresa",selWR.shipper||"—"],["Dirección","—"],["Tel / Email","—"]].map(([l,v])=><div key={l} className="wf"><div className="wfl">{l}</div><div className="wfv">{v}</div></div>)}
            </div>
            <div className="wb"><div className="wb-t">📥 Información del Consignatario</div>
              {[["Nombre",selWR.consignee],["Casillero",selWR.casillero],["País / Ciudad",`${selWR.destCountry?.split(" ")[0]} / ${selWR.destCity}`]].map(([l,v])=><div key={l} className="wf"><div className="wfl">{l}</div><div className="wfv" style={l==="Casillero"?{color:"var(--gold2)",fontWeight:700}:{}}>{v}</div></div>)}
            </div>
          </div>
          ...
          <div className="wb" style={{marginBottom:12}}>
            ...
            {selWR.dims&&selWR.dims.length>0&&(
              <>
                <div style={{display:"grid",gridTemplateColumns:"20px 1fr 1fr 1fr 1fr 1fr 1fr 1fr 1fr 2fr 1fr",gap:4,marginBottom:4,padding:"4px 0",borderBottom:"2px solid var(--navy)"}}>
                  {["#","Largo","Ancho","Alto","Peso","P.Vol.lb","P.Vol.kg","Ft³","M³","Tracking","Carrier"].map(h=>(
                ...
                {selWR.dims.map((d,i)=>{
                  ...
                  return (
                    <div key={i} style={{display:"grid",gridTemplateColumns:"20px 1fr 1fr 1fr 1fr 1fr 1fr 1fr 1fr 2fr 1fr",gap:4,padding:"6px 0",borderBottom:"1px solid var(--b2)",alignItems:"center"}}>
                      <div style={{fontWeight:700,color:"var(--navy)",fontSize:11}}>{i+1}</div>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:11}}>{toIn(d.l)}"</div>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:11}}>{toIn(d.a)}"</div>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:11}}>{toIn(d.h)}"</div>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,fontWeight:600}}>{toLb(d.pk)}lb</div>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"var(--orange)",fontWeight:700}}>{dv.volLb}lb</div>
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"var(--orange)"}}>{dv.volKg}kg</div>
                      ...
                      <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"var(--cyan)",fontWeight:600}}>{trk}</div>
          ...
          {canStatus&&(
            <div style={{marginBottom:12}}>
              ...
              <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{WR_STATUSES.map(s=><button key={s.code} className={`btn-${selWR.status.code===s.code?"p":"s"}`} style={{fontSize:9,padding:"3px 8px"}} onClick={()=>setSelWR(p=>({...p,status:s}))}>{s.code} {s.label}</button>)}</div>
            </div>
          )}
          <div className="wr-legal">
            Certifico que este envío no contiene dinero, narcóticos, armas o dispositivos explosivos no autorizados. <strong>ENEX International Courier</strong> no se hace responsable de los artículos no retirados en los treinta (30) días siguientes a su recepción. Nuestra responsabilidad en siniestros será de $100 por recibo si el cliente no asegura la carga. SHIPPER autoriza a ENEX a inspeccionar la carga de acuerdo con el Programa IACSSP aprobado por la TSA. Al recibir este documento, da fe de haber leído y estar de acuerdo con los términos y regulaciones.
          </div>
          <div className="wr-sigs"><div className="wr-sig">Firma y Nombre del Receptor · Fecha</div><div className="wr-sig">ENEX · Responsable de Recepción · Sello</div></div>
        </div>
        <div className="mft">
            ...
            <tbody>
              {consolList.map(c=>(
                <tr key={c.id}>
                  ...
                  <td><span className="st s3"><span className="st-dot"/>{c.status}</span></td>
                  <td>
      ...
      {showNewConsol&&(
        <div className="ov" onClick={()=>setShowNewConsol(false)}>
          <div className="modal mxl" onClick={e=>e.stopPropagation()}>
            ...
            <div className="mft">
              <button className="btn-s" onClick={()=>{setShowNewConsol(false);setCf(emptyConsol());setContScanVal({});setContScanErr({});}}>Cancelar</button>
    ...
    return (
      <div className="page-scroll">
        ...
        <div style={{display:"flex",gap:4,marginBottom:16,flexWrap:"wrap"}}>
          {CFG_TABS.map(t=>(
            <button key={t.k} onClick={()=>setCfgTab(t.k)}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            ...
            <div style={{background:"var(--bg2)",border:"1px solid var(--b1)",borderRadius:10,padding:"16px"}}>
              ...
              {[
                ...
                {v:2,l:"Tipo 2 — Indicativo + Oficinas + Secuencia",ex:`${empresaSlug}01MI5801MI0000001`},
                {v:3,l:"Tipo 3 — País+Ciudad Origen+Destino + Secuencia",ex:"01MI58VL0000001"},
              ].map(t=>(
              ...
              <div style={{marginTop:10,padding:"8px 12px",background:"var(--bg4)",borderRadius:6,fontFamily:"'DM Mono',monospace",fontSize:12,color:"var(--navy)",fontWeight:700}}>
                Próximo WR: {buildWRNum(OFFICE_CONFIG.origCountry,OFFICE_CONFIG.origCity,OFFICE_CONFIG.destCountry,OFFICE_CONFIG.destCity,wrList.length+1,wrNumTipo,wrSecInicio,empresaSlug)}
              </div>
            </div>
            ...
            <div style={{background:"var(--bg2)",border:"1px solid var(--b1)",borderRadius:10,padding:"16px"}}>
              ...
              {[
                {v:1,l:"Tipo 1 — 7 dígitos secuenciales",ex:"0000001"},
                {v:2,l:"Tipo 2 — País Orig+Dest + Secuencia + Tipo Envío",ex:"015800000001AE"},
                {v:3,l:"Tipo 3 — Fecha + Ciudad Destino + Secuencia",ex:"260320VL0000001"},
              ].map(t=>(
              ...
              <div style={{marginTop:10,padding:"8px 12px",background:"var(--bg4)",borderRadius:6,fontFamily:"'DM Mono',monospace",fontSize:12,color:"var(--navy)",fontWeight:700}}>
                Próxima Guía: {buildConsolNum("VL",consolList.length+1,consolNumTipo,consolSecInicio,OFFICE_CONFIG.origCountry,OFFICE_CONFIG.destCountry,"AE")}
              </div>
            </div>
            ...
            <div style={{background:"var(--bg2)",border:"1px solid var(--b1)",borderRadius:10,padding:"16px",gridColumn:"1/-1"}}>
              ...
              <div className="fg" style={{marginTop:12,maxWidth:300}}>
                ...
                <input className="fi" value={empresaNombre} onChange={e=>setEmpresaNombre(e.target.value)} placeholder="ENEX"/>
              </div>
            </div>
          </div>
        )}
        ...
        {cfgTab==="tipoenvio"&&(
          <ListEditor title="✈️ Tipos de Envío" items={SEND_TYPES}
            onAdd={v=>setSendTypes(p=>[...p,v])}
            onDelete={i=>setSendTypes(p=>p.filter((_,j)=>j!==i))}
        ...
        {cfgTab==="servicios"&&(
          <ListEditor title="⚡ Servicios Adicionales" items={CHARGES_OPT}
            onAdd={v=>setChargesOpt(p=>[...p,v])}
            onDelete={i=>setChargesOpt(p=>p.filter((_,j)=>j!==i))}
        ...
        {cfgTab==="pagos"&&(
          <ListEditor title="💳 Tipos de Pago" items={PAY_TYPES}
            onAdd={v=>setPayTypes(p=>[...p,v])}
            onDelete={i=>setPayTypes(p=>p.filter((_,j)=>j!==i))}
        ...
        {cfgTab==="contenedores"&&(
          <div style={{background:"var(--bg2)",border:"1px solid var(--b1)",borderRadius:10,padding:"14px 16px",marginBottom:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"100px 1fr 1fr 120px auto",gap:8,marginBottom:12,alignItems:"flex-end"}}>
              ...
              <div className="fg"><div className="fl">Nombre</div><input className="fi" value={cfgNewCont.name} onChange={e=>setCfgNewCont(p=>({...p,name:e.target.value}))} placeholder="EH — Extra High"/></div>
              <div className="fg"><div className="fl">Dimensiones</div><input className="fi" value={cfgNewCont.dim} onChange={e=>setCfgNewCont(p=>({...p,dim:e.target.value}))} placeholder="88×56×60 in"/></div>
              ...
              <button className="btn-p" style={{padding:"8px 14px",alignSelf:"flex-end"}} onClick={()=>{
                if(cfgNewCont.code&&cfgNewCont.name){
                  setContainerTypes(p=>[...p,{...cfgNewCont,maxLb:parseInt(cfgNewCont.maxLb)||0}]);
              ...
              <tbody>
                {CONTAINER_TYPES.map((c,i)=>(
                  <tr key={i}>
                    ...
                    <td style={{fontFamily:"'DM Mono',monospace",fontWeight:600}}>{c.maxLb.toLocaleString()}lb</td>
                    <td><button className="btn-d" style={{fontSize:10,padding:"2px 8px"}} onClick={()=>setContainerTypes(p=>p.filter((_,j)=>j!==i))}>🗑 Eliminar</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
                ...
                <div style={{display:"grid",gridTemplateColumns:"60px 1fr auto",gap:6,alignItems:"flex-end"}}>
                  ...
                  <button className="btn-p" style={{padding:"6px 10px",fontSize:11,alignSelf:"flex-end"}} onClick={()=>{
                    if(cfgNewPais.dial&&cfgNewPais.name){
                      setCountries(p=>[...p,{dial:cfgNewPais.dial,name:cfgNewPais.name,cities:[]}]);
              ...
              <div style={{maxHeight:400,overflowY:"auto"}}>
                {COUNTRIES.map((c,i)=>(
                  <div key={i} onClick={()=>setCfgSelPais(i)}
                    ...
                    <span onClick={e=>{e.stopPropagation();setCountries(p=>p.filter((_,j)=>j!==i));if(cfgSelPais>=i)setCfgSelPais(Math.max(0,cfgSelPais-1));}} style={{color:"var(--red)",cursor:"pointer",fontSize:13,fontWeight:700}}>×</span>
                  </div>
                ))}
              </div>
            </div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"80px 1fr auto",gap:6,alignItems:"flex-end"}}>
                      ...
                      <button className="btn-p" style={{padding:"6px 12px",alignSelf:"flex-end"}} onClick={()=>{
                        if(cfgNewCity.code&&cfgNewCity.name){
                          setCountries(p=>p.map((c,i)=>i===cfgSelPais?{...c,cities:[...c.cities,{code:cfgNewCity.code,name:cfgNewCity.name}]}:c));
                  ...
                  <div style={{padding:"8px 12px",display:"flex",flexWrap:"wrap",gap:6}}>
                    {(COUNTRIES[cfgSelPais]?.cities||[]).map((c,ci)=>(
                      <div key={ci} style={{display:"inline-flex",alignItems:"center",gap:5,background:"var(--bg4)",border:"1px solid var(--b1)",borderRadius:6,padding:"4px 10px",fontSize:12}}>
                        ...
                        <span onClick={()=>setCountries(p=>p.map((co,i)=>i===cfgSelPais?{...co,cities:co.cities.filter((_,j)=>j!==ci)}:co))} style={{color:"var(--red)",cursor:"pointer",fontSize:14,fontWeight:700}}>×</span>
                      </div>
                    ))}
                  ...
                  {agentes.map(a=>(
                    <tr key={a.id}>
                      ...
                      <td><div style={{display:"flex",gap:4}}>
                        ...
                        {a.codigo!=="01"&&<span className="ic-b" style={{color:"var(--red)"}} onClick={()=>setAgentes(p=>p.filter(x=>x.id!==a.id))}>🗑</span>}
                      </div></td>
                    </tr>
                  ))}
                </tbody>
            ...
            {showNewAgente&&(
              <div className="ov" onClick={()=>setShowNewAgente(false)}>
                <div className="modal" style={{maxWidth:520}} onClick={e=>e.stopPropagation()}>
                  ...
                  <div className="fgrid g2" style={{marginBottom:10}}>
                    <div className="fg full"><div className="fl">Nombre del Agente *</div><input className="fi" value={agForm.nombre} onChange={e=>setAgForm(p=>({...p,nombre:e.target.value}))} placeholder="Nombre completo o razón social"/></div>
                    <div className="fg"><div className="fl">Ciudad</div><input className="fi" value={agForm.ciudad} onChange={e=>setAgForm(p=>({...p,ciudad:e.target.value}))}/></div>
                    <div className="fg"><div className="fl">País</div><input className="fi" value={agForm.pais} onChange={e=>setAgForm(p=>({...p,pais:e.target.value}))}/></div>
                    <div className="fg"><div className="fl">Teléfono</div><input className="fi" value={agForm.tel} onChange={e=>setAgForm(p=>({...p,tel:e.target.value}))}/></div>
                    <div className="fg"><div className="fl">Email</div><input className="fi" value={agForm.email} onChange={e=>setAgForm(p=>({...p,email:e.target.value}))}/></div>
                    <div className="fg full"><div className="fl">Persona de Contacto</div><input className="fi" value={agForm.contacto} onChange={e=>setAgForm(p=>({...p,contacto:e.target.value}))}/></div>
                    <div className="fg full"><div className="fl">Notas</div><input className="fi" value={agForm.notas} onChange={e=>setAgForm(p=>({...p,notas:e.target.value}))}/></div>
                  </div>
                  <div className="mft">
                    ...
                    <button className="btn-p" onClick={()=>{
                      ...
                      if(editAgente){setAgentes(p=>p.map(a=>a.id===editAgente.id?{...a,...agForm}:a));}
                      else{const nextCod=String(agentes.length+1).padStart(2,"0");const na={...agForm,id:`AG-${nextCod}`,codigo:nextCod,activo:true};setAgentes(p=>[...p,na]);dbUpsertAgente(na);}
                      if(editAgente)dbUpsertAgente({...editAgente,...agForm});
        ...
        {cfgTab==="oficinas"&&(
          <div>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
              ...
              <button className="btn-p" style={{fontSize:11}} onClick={()=>{setOfForm({nombre:"",ciudad:"",pais:"",tel:"",email:"",contacto:"",notas:""});setShowNewOficina(true);}}>+ Nueva Oficina</button>
            </div>
            <div style={{background:"var(--bg2)",border:"1px solid var(--b1)",borderRadius:10,overflow:"hidden"}}>
              <table className="ct">
                <thead><tr>
                ...
                <tbody>
                  {oficinas.length===0&&<tr><td colSpan={9} style={{textAlign:"center",padding:30,color:"var(--t3)"}}>No hay oficinas registradas aún.</td></tr>}
                  {oficinas.map(o=>(
                    <tr key={o.id}>
                      ...
                      <td><div style={{display:"flex",gap:4}}>
                        ...
                        <span className="ic-b" style={{color:"var(--red)"}} onClick={()=>setOficinas(p=>p.filter(x=>x.id!==o.id))}>🗑</span>
                      </div></td>
                    </tr>
                  ))}
                </tbody>
            ...
            {showNewOficina&&(
              <div className="ov" onClick={()=>setShowNewOficina(false)}>
                <div className="modal" style={{maxWidth:520}} onClick={e=>e.stopPropagation()}>
                  ...
                  <div className="fgrid g2" style={{marginBottom:10}}>
                    <div className="fg full"><div className="fl">Nombre de la Oficina *</div><input className="fi" value={ofForm.nombre} onChange={e=>setOfForm(p=>({...p,nombre:e.target.value}))} placeholder="Ej: Oficina Valencia"/></div>
                    <div className="fg"><div className="fl">Ciudad</div><input className="fi" value={ofForm.ciudad} onChange={e=>setOfForm(p=>({...p,ciudad:e.target.value}))}/></div>
                    <div className="fg"><div className="fl">País</div><input className="fi" value={ofForm.pais} onChange={e=>setOfForm(p=>({...p,pais:e.target.value}))}/></div>
                    <div className="fg"><div className="fl">Teléfono</div><input className="fi" value={ofForm.tel} onChange={e=>setOfForm(p=>({...p,tel:e.target.value}))}/></div>
                    <div className="fg"><div className="fl">Email</div><input className="fi" value={ofForm.email} onChange={e=>setOfForm(p=>({...p,email:e.target.value}))}/></div>
                    <div className="fg full"><div className="fl">Persona de Contacto</div><input className="fi" value={ofForm.contacto} onChange={e=>setOfForm(p=>({...p,contacto:e.target.value}))}/></div>
                    <div className="fg full"><div className="fl">Notas</div><input className="fi" value={ofForm.notas} onChange={e=>setOfForm(p=>({...p,notas:e.target.value}))}/></div>
                  </div>
                  <div className="mft">
                    <button className="btn-s" onClick={()=>{setShowNewOficina(false);setEditOficina(null);}}>Cancelar</button>
                    <button className="btn-p" onClick={()=>{
                      if(!ofForm.nombre.trim())return;
                      if(editOficina){setOficinas(p=>p.map(o=>o.id===editOficina.id?{...o,...ofForm}:o));}
                      else{const nextCod=String(oficinas.length+1).padStart(2,"0");const no={...ofForm,id:`OF-${nextCod}`,codigo:nextCod,activo:true};setOficinas(p=>[...p,no]);dbUpsertOficina(no);}
                      if(editOficina)dbUpsertOficina({...editOficina,...ofForm});
        ...
        {cfgTab==="tarifas"&&(
          <div>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
              ...
              <button className="btn-p" style={{fontSize:11}} onClick={()=>{setTafForm({paisOrig:"01",ciudadOrig:"MI",paisDest:"58",ciudadDest:"VL",tipoEnvio:"Aéreo Express",porLb:"",porFt3:"",min:"",moneda:"USD"});setEditTarifa(null);setShowNewTarifa(true);}}>+ Nueva Tarifa</button>
            </div>
            <div style={{background:"var(--bg2)",border:"1px solid var(--b1)",borderRadius:10,overflow:"hidden"}}>
              <table className="ct">
                <thead><tr>
                  <th>ID</th><th>Origen</th><th>Destino</th><th>Tipo Envío</th><th>$/lb</th><th>$/ft³</th><th>Mínimo</th><th>Moneda</th><th>Estado</th><th>Acc.</th>
                </tr></thead>
                <tbody>
                  {tarifas.map(t=>(
                    <tr key={t.id}>
                      <td style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"var(--t3)"}}>{t.id}</td>
                      ...
                      <td style={{fontFamily:"'DM Mono',monospace",fontWeight:700,color:"var(--green)"}}>${t.porLb}</td>
                      <td style={{fontFamily:"'DM Mono',monospace",fontWeight:700,color:"var(--sky)"}}>${t.porFt3}</td>
                      ...
                      <td><div style={{display:"flex",gap:4}}>
                        ...
                        <span className="ic-b" style={{color:"var(--red)"}} onClick={()=>setTarifas(p=>p.filter(x=>x.id!==t.id))}>🗑</span>
                      </div></td>
                    </tr>
                  ))}
                </tbody>
            ...
            {showNewTarifa&&(
              <div className="ov" onClick={()=>setShowNewTarifa(false)}>
                <div className="modal" style={{maxWidth:560}} onClick={e=>e.stopPropagation()}>
                  ...
                  <div className="fgrid g4" style={{marginBottom:10}}>
                    <div className="fg"><div className="fl">País Origen</div><input className="fi" style={{fontFamily:"'DM Mono',monospace"}} value={tafForm.paisOrig} onChange={e=>setTafForm(p=>({...p,paisOrig:e.target.value.toUpperCase()}))} placeholder="01"/></div>
                    <div className="fg"><div className="fl">Ciudad Origen</div><input className="fi" style={{fontFamily:"'DM Mono',monospace"}} value={tafForm.ciudadOrig} onChange={e=>setTafForm(p=>({...p,ciudadOrig:e.target.value.toUpperCase()}))} placeholder="MI"/></div>
                    <div className="fg"><div className="fl">País Destino</div><input className="fi" style={{fontFamily:"'DM Mono',monospace"}} value={tafForm.paisDest} onChange={e=>setTafForm(p=>({...p,paisDest:e.target.value.toUpperCase()}))} placeholder="58"/></div>
                    <div className="fg"><div className="fl">Ciudad Destino</div><input className="fi" style={{fontFamily:"'DM Mono',monospace"}} value={tafForm.ciudadDest} onChange={e=>setTafForm(p=>({...p,ciudadDest:e.target.value.toUpperCase()}))} placeholder="VL"/></div>
                  </div>
                  <div className="sdiv">TARIFA</div>
                  <div className="fgrid g3" style={{marginBottom:10}}>
                    <div className="fg"><div className="fl">Tipo de Envío</div><select className="fs" value={tafForm.tipoEnvio} onChange={e=>setTafForm(p=>({...p,tipoEnvio:e.target.value}))}>{SEND_TYPES.map(t=><option key={t} value={t}>{t}</option>)}</select></div>
                    <div className="fg"><div className="fl">Precio por lb ($)</div><input className="fi" type="number" step="0.01" value={tafForm.porLb} onChange={e=>setTafForm(p=>({...p,porLb:e.target.value}))}/></div>
                    <div className="fg"><div className="fl">Precio por ft³ ($)</div><input className="fi" type="number" step="0.01" value={tafForm.porFt3} onChange={e=>setTafForm(p=>({...p,porFt3:e.target.value}))}/></div>
                    <div className="fg"><div className="fl">Cargo mínimo ($)</div><input className="fi" type="number" step="0.01" value={tafForm.min} onChange={e=>setTafForm(p=>({...p,min:e.target.value}))}/></div>
                  ...
                  <div className="mft">
                    ...
                    <button className="btn-p" onClick={()=>{
                      if(!tafForm.tipoEnvio||!tafForm.porLb)return;
                      if(editTarifa){setTarifas(p=>p.map(t=>t.id===editTarifa.id?{...t,...tafForm,porLb:parseFloat(tafForm.porLb),porFt3:parseFloat(tafForm.porFt3),min:parseFloat(tafForm.min)}:t));}
                      else{const nid=`T-${String(tarifas.length+1).padStart(3,"0")}`;const nt={...tafForm,id:nid,porLb:parseFloat(tafForm.porLb),porFt3:parseFloat(tafForm.porFt3),min:parseFloat(tafForm.min),activo:true};setTarifas(p=>[...p,nt]);dbUpsertTarifa(nt);}
                      if(editTarifa)dbUpsertTarifa({...editTarifa,...tafForm,porLb:parseFloat(tafForm.porLb),porFt3:parseFloat(tafForm.porFt3),min:parseFloat(tafForm.min)});
  ...
  const trkCanUpdate=["A","B","C","D","D1","D2"].includes(CU.rol);
  const trkCanOrigin=["A","B","C","D","D1"].includes(CU.rol);   // estados 1-7
  const trkCanDest=["A","B","C","D","D2"].includes(CU.rol);     // estados 8-12P

  const statusAllowed=(st)=>{
    const cod=parseFloat(st.code);
    if(["A","B","C","D"].includes(CU.rol)) return true;
    if(CU.rol==="D1") return cod<=7.1;
    if(CU.rol==="D2") return cod>=8;
    return false;
  };
  ...
  const applyStatus=(wr,newSt,nota="")=>{
    setWrList(p=>p.map(w=>{
      if(w.id===wr.id){
        const updated={...w,status:newSt,historial:[...(w.historial||[]),{status:newSt,fecha:new Date(),user:CU.id||"rdiaz",nota}]};
        dbUpsertWR(updated);
        return updated;
      }
      return w;
    ...
    if(trkSelected?.id===wr.id) setTrkSelected(p=>({...p,status:newSt,historial:[...(p.historial||[]),{status:newSt,fecha:new Date(),user:CU.id||"rdiaz",nota}]}));
  };
  ...
  const sendChat=()=>{
    ...
    const msg={id:Date.now(),user:CU.id||"rdiaz",role:CU.rol,name:`${CU.primerNombre} ${CU.primerApellido}`,text:chatMsg.trim(),ts:new Date()};
      ...
      <div style={{display:"grid",gridTemplateColumns:"220px 1fr",flex:1,minHeight:0,overflow:"hidden"}}>
        ...
        <div style={{background:"var(--navy)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div style={{padding:"10px 12px",borderTop:"1px solid rgba(255,255,255,.1)",display:"flex",alignItems:"center",gap:8}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:12,fontWeight:600,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{CU.primerNombre} {CU.primerApellido}</div>
              <div style={{fontSize:10,color:"rgba(255,255,255,.4)"}}>{ROLE_DEFS[CU.rol]?.name}</div>
            </div>
          </div>
        </div>
  ...
  const calcResult=(()=>{
    ...
    const tarifa=TARIFAS_BASE[calcForm.tipoEnvio]||TARIFAS_BASE["Aéreo Express"];
  ...
  const generarFacturasGuia=(guia)=>{
    ...
    const nuevas=Object.values(porCliente).map((c,i)=>{
      ...
      const tarifa=TARIFAS_BASE[guia.tipoEnvio]||TARIFAS_BASE["Aéreo Express"];
    ...
    <div className="app" onClick={()=>setDimOpen(null)} translate="no" lang="en">
      ...
      <div className="sb">
        ...
        <div className="sb-foot">
          <div className="u-row">
            <div className="u-av">{initials(CU)}</div>
            <div style={{flex:1,minWidth:0}}><div className="u-nm">{CU.primerNombre} {CU.primerApellido}</div><div className="u-rl">{CU.email}</div></div>
            <RoleBadge code={CU.rol}/>
          </div>
        </div>
      </div>
      ...
      <div className="main">
        <div className="topbar">
          ...
          <div style={{width:28,height:28,borderRadius:6,background:"var(--navy)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,cursor:"pointer",color:"#fff"}}>{initials(CU)}</div>
        </div>
        {renderPage()}
      </div>
      ...
      {showNewCl&&<ClientModal agentes={agentes} oficinas={oficinas} allClients={clients} title="➕ Nuevo Registro" initial={{tipo:"cliente",primerNombre:"",segundoNombre:"",primerApellido:"",segundoApellido:"",cedula:"",dir:"",municipio:"",estado:"",pais:"",cp:"",tel1:"",tel2:"",email:"",casillero:"",rol:"I",password:""}} onClose={()=>setShowNewCl(false)} onSave={f=>{const esUser=f.tipo==="usuario";const prefix=esUser?"U":"C";const nextNum=clients.filter(c=>c.tipo===(esUser?"usuario":"cliente")).length+1;const newId=`${prefix}-${String(nextNum).padStart(3,"0")}`;const casillero=(!esUser&&!f.casillero)?newId:f.casillero;const newRec={...f,id:newId,casillero};setClients(p=>[...p,newRec]);dbUpsertCliente(newRec);setShowNewCl(false);logAction("Creó registro",`${newId} — ${f.primerNombre} ${f.primerApellido}`);}}/>}
      {showEditCl&&<ClientModal agentes={agentes} oficinas={oficinas} allClients={clients} title={`✏️ Editar — ${fullName(showEditCl)}`} initial={showEditCl} onClose={()=>setShowEditCl(null)} onSave={f=>{setClients(p=>p.map(c=>c.id===f.id?f:c));dbUpsertCliente(f);logAction("Editó registro",`${f.id} — ${f.primerNombre} ${f.primerApellido}`);setShowEditCl(null);}}/>}
    </div></>
  );
}
